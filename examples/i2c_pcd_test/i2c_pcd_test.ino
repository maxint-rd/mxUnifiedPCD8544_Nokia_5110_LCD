...
mxUnifiedPCD8544 display = mxUnifiedPCD8544(&unio, 7, 6, 5, 4, 2);    // when using the LCD expander, this are the best matching pins
...
//#define LED_BUILTIN 2   // On ESP-01 pin 1 (TX) is connected to the blue led, on ESP07, 12E and 12F this constant requires redefinition
#define PIN_BLINK_LED LED_BUILTIN   // On Arduino pin 13 is most often equiped with a LED. On ESP-01 it is pin 1 (TX), pin 2 on other ESP's.
#define EPIN_BACKLIGHT 3   // When using the LCD backboard PCF8574 module, expanded pin 3 is the best match for the backlight
...
void delayBlink(int nDelay, int nPin=PIN_BLINK_LED)
{ // delay some msecs and blink on/off at half delay interval
  unio.digitalWrite(EPIN_BACKLIGHT, HIGH);
  digitalWrite(nPin, HIGH);
  delay(nDelay/2);
  unio.digitalWrite(EPIN_BACKLIGHT, LOW);
...
void setup()
{
  ...
  Serial.begin(115200);             // For ESP-01 board: Disable Serial.begin when using GPIO 1 and 3 for I2C.
  delay(100);
  Serial.println(F("\n\n"));
  ...
  unio.begin();   // initialize the I2C connection with default I2C speed. You can specify an alternative speed. (On ESP8266 only applicable to hardware I2C)
  //unio.begin(1000000L); // regular begin() uses high speeed (1MHz on ESP8266, 400kHz on others)
